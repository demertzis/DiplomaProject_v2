...
offset_model_dir = 'pretrained_networks/model_output_8_offset_3.keras'
...
offset_q_net, offset_target_q_net = load_pretrained_model(offset_model_dir)
...
agent_list = []
offset = False
agents_cases = [3, 6, 9, 15, 30]
for num_of_agents in agents_cases:
    for i in range(num_of_agents):
        ...
        agent_list.append(create_single_agent(cls=DdqnAgent,
                                              ckpt_dir=ckpt_dir,
                                              vehicle_distribution=list(vehicles) if not offset else list(offset_vehicles),
    ...
    # new_model = tf.keras.models.Sequential(agent_list[0]._q_network.layers)
    # new_model.build((1,34))
    # model.save('pretrained_networks/model_output_8_offset.keras')
    # def create_data():
