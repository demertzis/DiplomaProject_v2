...
class Vehicle:
    ...
    """
    @tf.function
    def park(self, max_charging_rate: tf.Tensor, max_discharging_rate: tf.Tensor, vehicle: tf.Tensor):
        """
    ...
    @tf.function
    def _calculate_next_max_charge(self, current_charge: tf.Tensor,
    ...
    @tf.function
    def _calculate_next_min_charge(self,
                                   current_charge: tf.Tensor,
                                   time_before_departure: tf.Tensor,
                                   vehicle: tf.Tensor):
        """
    ...
    @tf.function
    def _update_next_charging_states(self, vehicle: tf.Tensor):
        """
        ...
        next_max_charge = self._calculate_next_max_charge(vehicle[0],
                                                          vehicle[2],
                                                          vehicle)
        next_min_charge = self._calculate_next_min_charge(vehicle[0],
                                                          vehicle[2],
                                                          vehicle)
        ...
        return tf.tensor_scatter_nd_update(vehicle,
                                           ...
                                           [new_max_charge,
                                            ...
                                            new_min_discharge])
    @tf.function
    def _calculate_charge_curves(self, vehicle: tf.Tensor):
        """
        ...
        current_max_charge = tf.expand_dims(vehicle[0], 0)
        current_min_charge = current_max_charge
        time_before_departure = vehicle[2]
        i = tf.constant(1)
        c = lambda i, cmxc, cmnc: i > 0.0
        b = lambda i, cmxc, cmnc: (i - 1.0,
                                   tf.concat((cmxc, tf.expand_dims(self._calculate_next_max_charge(cmxc[-1], i, vehicle), axis=0)), 0),
                                   tf.concat((cmnc, tf.expand_dims(self._calculate_next_min_charge(cmnc[-1], i, vehicle), axis=0)), 0))
    ...
    @tf.function
    def _update_priorities(self, vehicle: tf.Tensor):
        """
        ...
        max_curve, min_curve = self._calculate_charge_curves(vehicle)
    ...
    @tf.function
    def update(self, vehicle: tf.Tensor):
        """
        Update state variables
        """
        print('Tracing update')
        v = vehicle
        v = self._update_next_charging_states(v)
        return self._update_priorities(v)
    ...
    def update_current_charge(self,
        ...
        new_vehicle = tf.cond(tf.not_equal(new_vehicle[2], 0.0),
                              lambda: self.update(new_vehicle),
                              lambda: new_vehicle)

        return residue, new_vehicle

    @tf.function
    def update_emergency_demand(self, vehicle: tf.Tensor):
        """
