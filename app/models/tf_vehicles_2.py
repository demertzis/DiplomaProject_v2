...
def vehicle_dict(
    ...
    min_charge,):
    return VehicleDict(
        _current_charge=current_charge,
        _target_charge=target_charge,
        _time_before_departure=time_before_departure,
        _max_charge=max_charge,
        _min_charge=min_charge,
        _charge_priority=tf.constant(0.0, dtype=tf.float32),
        _discharge_priority=tf.constant(0.0, dtype=tf.float32),)

class Vehicle:
    # NamedTuple('Vehicle', [('current_charge', types.TensorSpec),
    #                        ('target_charge', types.TensorSpec),
    #                        ('time_before_departure', types.TensorSpec),
    #                        ('max_charge', types.TensorSpec),
    #                        ('min_charge', types.TensorSpec),
    #                        ('charge_priority', types.TensorSpec),
    #                        ('discharge_priority', types.TensorSpec),])):
    """
    ...
    def _calculate_next_max_charge(self, current_charge: float, time_before_departure: int):
        ...
        return my_round(
            min(
                ...
                self._max_charge,
                float(time_before_departure - 1) * self._max_discharging_rate + self._target_charge,
            ),
            3,
        )
    ...
    def _calculate_next_min_charge(self, current_charge: float, time_before_departure: int):
        ...
        return my_round(
            max(
                ...
                self._min_charge,
                self._target_charge - float(time_before_departure - 1) * self._max_charging_rate,
            ),
            3,
        )
    ...
    def _update_next_charging_states(self):
        ...
        self._next_max_charge = my_round(max(0, next_max_charge - self._current_charge), 3)
        self._next_min_charge = my_round(max(0, next_min_charge - self._current_charge), 3)
        self._next_max_discharge = my_round(max(0, self._current_charge - next_min_charge), 3)
        self._next_min_discharge = my_round(max(0, self._current_charge - next_max_charge), 3)
    ...
    def _update_priorities(self):
        ...
        try:
            ...
            self._charge_priority = my_round(self._charge_priority, 2)
            self._discharge_priority = my_round(self._discharge_priority, 2)
