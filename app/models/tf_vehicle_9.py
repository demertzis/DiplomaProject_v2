...
def _update_priorities(vehicles: tf.Tensor, num_of_vehicles: int):
    ...
    # where_y_2 = tf.where(tf.less_equal(current_charge, target_charge),
    ...
    # pr_2 = tf.where(tf.less(0.0, chosen_intersections), where_x, where_y)
    ...
    # pr_c_2 = tf.where(tf.math.logical_or(tf.not_equal(pr, 0.0), tf.not_equal(current_charge, target_charge)),
    #                 1.0 - pr,
    #                 tf.zeros_like(current_charge, tf.float32))
    ...
    # final_probs_update_2 = tf.where(tf.less(0, intersection_gather_index[..., :1]),
