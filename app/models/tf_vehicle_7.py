...
def park(vehicles: tf.Tensor):
    ...
    correct_mult = tf.expand_dims(tf.where(tf.less(0.0, new_vehicles[..., 2]), 1.0, 0.0), axis=1)
    return update(new_vehicles * correct_mult)

# @tf.function(input_signature=[tf.TensorSpec([None], tf.float32, _name='Current_Charge'),
#                               tf.TensorSpec([None], tf.float32, _name='Time_Before_Departure'),
#                               tf.TensorSpec([None, 13], tf.float32, _name='Vehicles')])
...
def _calculate_charge_curves(vehicles: tf.Tensor):
    ...
    temp = tf.repeat(tf.expand_dims(tf.range(13.0), 0), vehicles.shape[0], axis=0)
    t = tf.where(tf.equal(0.0,
                          temp - vehicles[..., 2:3]),
                 1.0,
                 0.0)
