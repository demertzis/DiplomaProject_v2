...
class TFPowerMarketEnv(TFEnvironment, tf.Module):
    def __init__(self,
        ...
        time_step_spec = TimeStep(step_type=tensor_spec.add_outer_dim(single_agent_time_step_spec.step_type, 1),
                                  ...
                                  observation=tensor_spec.BoundedTensorSpec(shape=(14,),
                                                                            ...
                                                                            minimum=-1.,
                                                                            maximum=1.))
        action_spec = tensor_spec.TensorSpec(shape=(number_of_agents,), dtype=tf.float32, name="action")
        ...
        self._last_time_step = tf.Variable([0.0] * (1 + 1 + number_of_agents + self.observation_spec().shape[0]), dtype=tf.float32, trainable=False)
    ...
    def _get_obs(self, time):
        ...
        return tf.ensure_shape(tf.concat((da_prices, id_price, [1.0] - tf.cast(time, tf.float32) / 24.0), axis=0),
