...
class Parking:
    ...
    _max_charging_rate = tf.constant(cfg.MAX_CHARGING_RATE, tf.float16).numpy()
    _max_discharging_rate = tf.constant(cfg.MAX_DISCHARGING_RATE, tf.float16).numpy()

    def __init__(self, capacity: int, name: str):
        self._capacity = tf.constant(capacity, tf.int64)
        self._capacity_int32 = capacity
        self._vehicles = tf.Variable(tf.zeros(shape=(capacity, 13), dtype=tf.float16), trainable=False)
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=[None, 13], dtype=tf.float16)])
    ...
    # @tf.function
    def _calculate_normalization_constant(self, vehicle_array):
        #print('Tracing _calculate_normalization_constant')
        # vehicle_array = self._vehicles
        needed_fields_1 = tf.cast(vehicle_array[..., 9:12:2], tf.float32)
        needed_fields_2 = tf.cast(vehicle_array[..., 5:7], tf.float32)
        norm_constants = tf.reduce_sum(needed_fields_1 * needed_fields_2, axis=0)
        # normalization_charge_constant = my_round(tf.math.divide_no_nan(norm_constants[0],
        #                                                                self._next_max_charge),
        #                                          tf.constant(3))
        normalization_charge_constant = tf.math.divide_no_nan(norm_constants[0],
                                                              self._next_max_charge)
        # normalization_discharge_constant = my_round(tf.math.divide_no_nan(norm_constants[1],
        #                                                                   self._next_max_discharge),
        #                                             tf.constant(3))
        normalization_discharge_constant = tf.math.divide_no_nan(norm_constants[1],
                                                                 self._next_max_discharge)
        return tf.cast(normalization_charge_constant, tf.float16), tf.cast(normalization_discharge_constant, tf.float16)
    ...
    def update_energy_state(self, charging_coefficient: tf.Tensor, vehicles: tf.Tensor):
        ...
        is_charging = tf.where(tf.less(zero_16, charging_coefficient), one_16, tf.negative(one_16))
        ...
        normalization_charge_constant, normalization_discharge_constant = self._calculate_normalization_constant(vehicle_array)
        normalization_constant = tf.math.maximum(normalization_charge_constant * is_charging,
                                                 normalization_discharge_constant * tf.math.negative(is_charging))
