...
import app.models.tf_vehicle_7 as Vehicle
...
class Parking:
    ...
    def assign_vehicles(self, vehicles: tf.Tensor):
        ...
        new_charges = tf.reduce_sum(vehicles_added[..., 0])
    ...
    def depart_vehicles(self):
        ...
        mult_tensor = tf.where(tf.less(0.0, vehicle_list[..., 2]), 1.0, 0.0)
    ...
    def _calculate_normalization_constant(self):
        ...
        needed_fields_1 = vehicle_array[..., 9:12:2]
        needed_fields_2 = vehicle_array[..., 5:7]
        ...
        normalization_chargeupdate_current_charge_constant = my_round(tf.math.divide_no_nan(normalization_charge_constant,
    ...
    def update_energy_state(self, charging_coefficient: tf.Tensor):
        ...
        vehicle_array = Vehicle.update_emergency_demand(self._vehicles)
    ...
    def _sort_vehicles_by_departure(self, vehicle_array):
        ...
        permutation_sparse_matrix = tf.sparse.SparseTensor(indice_tensor,
                                                           tf.ones([self._capacity], tf.float32),
    ...
    # @tf.function(input_signature=[tf.TensorSpec(shape=[None, 13], dtype=tf.float32, _name='vehicle_array'),
    #                               tf.TensorSpec(shape=(), dtype=tf.float32, _name='charging_coefficient'),
    #                               tf.TensorSpec(shape=(), dtype=tf.float32, _name='normalization_constant')])
    # @tf.function
    def _sort_vehicles_for_charge_update(self,
        ...
        tensor_1 = (1.0 + vehicle_array[..., 5] - normalization_constant)
        tensor_2 = (1.0 + vehicle_array[..., 6] - normalization_constant)
