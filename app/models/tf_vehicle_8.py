...
from app.models.tf_utils import tf_find_intersection_vectorized_f16, my_pad
...
def park(vehicles: tf.Tensor, max_charging_rate, max_discharging_rate, num_of_vehicles: int):
    ...
    correct_mult = tf.expand_dims(tf.where(tf.less(tf.constant(0.0, tf.float16), new_vehicles[..., 2]),
                                           tf.constant(1.0, tf.float16),
                                           tf.constant(0.0, tf.float16)),
                                  axis=1)
...
def _update_next_charging_states(vehicles: tf.Tensor):
    ...
    next_min_charge = _calculate_next_min_charge(vehicles[..., 0],
                                                 vehicles[..., 2],
                                                 vehicles)
    zeros = tf.zeros_like(next_max_charge, tf.float16)
...
def _calculate_charge_curves(vehicles: tf.Tensor, num_of_vehicles: int):
    ...
    temp = tf.repeat(tf.expand_dims(tf.cast(tf.range(13.0), tf.float16), 0), num_of_vehicles, axis=0)
    t = tf.where(tf.equal(tf.constant(0.0, tf.float16),
                          temp - vehicles[..., 2:3]),
                 tf.constant(1.0, tf.float16),
                 tf.constant(0.0, tf.float16))
...
def _update_priorities(vehicles: tf.Tensor, num_of_vehicles: int):
    ...
    x_axes = tf.cast(tf.range(13.0), tf.float16)
    ...
    intersections = tf_find_intersection_vectorized_f16(x_axes,
                                                        curves,
    ...
    intersection_gather_index = tf.stack((intersection_gather_index,
                                          vehicle_indices), 1)
    all_curves = tf.clip_by_value((curves - current_charge_expanded) * tf.constant([[[-1.0]], [[1.0]]], tf.float16),
                                  0.0,
                                  60.0)
    ...
    where_y = tf.where(tf.less_equal(current_charge, target_charge),
                       tf.zeros_like(current_charge, tf.float16),
                       tf.ones_like(current_charge, tf.float16))
    float_0 = tf.constant(0.0, tf.float16)
    float_1 = tf.constant(1.0, tf.float16)
    float_2 = tf.constant(2.0, tf.float16)
    pr = tf.where(tf.less(float_0, chosen_intersections), where_x, where_y)
    pr_c = tf.where(tf.math.logical_or(tf.not_equal(pr, float_0), tf.not_equal(current_charge, target_charge)),
                    float_1 - pr,
                    tf.zeros_like(current_charge, tf.float16))
    # pr = my_round_16(pr / tf.math.maximum(float_1, time_before_departure - float_2),
    #                          tf.constant(2))
    pr = pr / tf.math.maximum(float_1, time_before_departure - float_2)
    # pr_c = my_round_16(pr_c / tf.math.maximum(float_1, time_before_departure - float_2),
    #                          tf.constant(2))
    pr_c = pr_c / tf.math.maximum(float_1, time_before_departure - float_2)
...
def update_current_charge(charging_coefficient: tf.Tensor,
    ...
    final_energy = tf.clip_by_value(residue_cumsum, surplus_tensor, zero_16) + next_max_energy
