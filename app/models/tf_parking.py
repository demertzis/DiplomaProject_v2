...
class Parking:
    ...
    def assign_vehicle(self, vehicle: Vehicle):
        ...
        Vehicle.park(vehicle, self._max_charging_rate, self._max_discharging_rate)
    ...
    def _update_single_vehicle(self, vehicle: Vehicle, vehicles_counted: Optional[int] = None):
        ...
        self._next_max_charge += vehicle.get_next_max_charge()
        self._next_min_charge += vehicle.get_next_min_charge()
        self._next_max_discharge += vehicle.get_next_max_discharge()
        self._next_min_discharge += vehicle.get_next_min_discharge()
        ...
        self._charge_mean_priority = ((num_of_cars - 1) *
                                     ...
                                     vehicle.get_charge_priority()) /\
                                     num_of_cars
        self._discharge_mean_priority = ((num_of_cars - 1) *
                                        self._charge_mean_priority +
                                        vehicle.get_discharge_priority()) /\
                                        num_of_cars
    ...
    def update_energy_state(self, charging_coefficient: float):
        ...
        if is_charging:
            sorted_vehicles = sorted(
                self._vehicles,
                key=lambda v: sign * charging_coefficient * (1.0 + v.get_charge_priority() - normalization_constant),
                reverse=True,
            )
        else:
            sorted_vehicles = sorted(
                self._vehicles,
                key=lambda v: sign * charging_coefficient * (1.0 + v.get_discharge_priority() - normalization_constant),
                reverse=True,
            )
