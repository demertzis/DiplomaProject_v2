...
class MultiAgentPolicyWrapper(TFPolicy):
    def __init__(self,
        ...
        self._time_step_float_16_tensors = tf.Variable(tf.zeros((13,), tf.float16), trainable=False)
        self._time_step_float_32_tensors = tf.Variable(tf.zeros((self._num_of_agents + 1,), tf.float32), trainable=False)
    ...
    def _get_time_step(self):
        float_16_members = self._time_step_float_16_tensors
        ...
        observation = tf.expand_dims(float_16_members, axis=0)
    ...
    def _action(self, time_step: ts.TimeStep,
        ...
        float_time_16_step_members = time_step.observation[0]
        # float_time_32_step_members = time_step.reward[0]
        float_time_32_step_members = tf.concat((time_step.discount,
                                                time_step.reward[0]), axis=0)
        int_time_step_members = time_step.step_type[0]
        self._time_step_float_16_tensors.assign(float_time_16_step_members)
