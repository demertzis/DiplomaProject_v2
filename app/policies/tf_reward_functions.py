...
# def halfway_uniform(prices: tf.Tensor, crid_price: tf.Tensor, tick: tf.Tensor, action: tf.Tensor, avg_consumption: tf.Tensor) -> tf.Tensor:
# 	# total_demand = tf.math.reduce_sum(action)
# 	# demand_overflow = total_demand - avg_consumption
# 	# sell_price_min = crid_price * CRID_COEFFICIENT
# 	# vanilla_prices = vanilla(prices, crid_price, tick , action , avg_consumption)
# 	vanilla_prices = vanilla(prices, crid_price, tick, action, avg_consumption)
# 	total_demand = tf.math.reduce_sum(action)
...
def punishing_non_uniform(prices: tf.Tensor, crid_price: tf.Tensor, tick: tf.Tensor, action: tf.Tensor, avg_consumption: tf.Tensor) -> tf.Tensor:
    total_demand, buy_price, sell_price, sell_price_min, buy_price_max = _get_basic_values(prices,
                                                                                           crid_price,
                                                                                           tick,
                                                                                           action,
    ...
    filtered_exp_buy_ratios = exp_buy_ratios * tf.cast(tf.less(buyers_tensor, 0.0), tf.float32)
    ...
    sell_final_ratios = corrected_inverse_exp_sell_ratios * punishing_coefficient + \
                        ...
                        # tf.math.divide_no_nan(sell_ratios) * (1.0 - punishing_coefficient)
                        # tf.math.divide_no_nan(inverse_sell_ratios,
                        # 					  tf.reduce_sum(inverse_sell_ratios)) * (1.0 - punishing_coefficient)

    # ratios = tf.math.divide_no_nan(buy_ammount, buy_ammount - buyers_tensor)
    # sell_ratios = tf.math.divide_no_nan(sell_ammount, sell_ammount - sellers_tensor)
    # exp_ratios = tf.math.exp(sell_ratios) / tf.reduce_sum(tf.reduce_sum(tf.math.exp(sell_ratios)))
    #
    # buy_exp_tensor = tf.math.exp(tf.math.divide_no_nan(buyers_tensor, buy_ammount))
    # buy_exp_ratios = tf.math.divide_no_nan(buy_exp_tensor, tf.reduce_sum(buy_exp_tensor))
    # sell_exp_tensor = tf.math.exp(tf.math.divide_no_nan(tf.math.abs(sellers_tensor), tf.math.abs(sell_ammount)))
    # sell_exp_ratios = tf.math.divide_no_nan(sell_exp_tensor, tf.reduce_sum(sell_exp_tensor))
    # # baseline_buy_ammount = (tf.cast(tf.less(0.0, action), tf.float32) * final_buy_price * action
